...
def main_parser() -> argparse.ArgumentParser:
    ...
    lock_parser = subparsers.add_parser(
        'lock', description='Lock the vehicle.')
    _add_default_arguments(lock_parser)
    lock_parser.add_argument('vin', help=TEXT_VIN)
    lock_parser.set_defaults(func=remote_lock)

    unlock_parser = subparsers.add_parser(
        'unlock', description='Unlock the vehicle.')
    _add_default_arguments(unlock_parser)
    unlock_parser.add_argument('vin', help=TEXT_VIN)
    unlock_parser.set_defaults(func=remote_unlock)

    climatize_parser = subparsers.add_parser(
        'climatize', description='Climatize the vehicle.')
    _add_default_arguments(climatize_parser)
    climatize_parser.add_argument('vin', help=TEXT_VIN)
    climatize_parser.set_defaults(func=climatize)
    ...
    sendpoi_parser.add_argument(
        "latitude", help="Latitude of the POI", type=float)
    sendpoi_parser.add_argument(
        "longitude", help="Longitude of the POI", type=float)
    sendpoi_parser.add_argument(
        "--name", help="(optional, display only) Name of the POI", nargs="?", default=None)
    sendpoi_parser.add_argument(
        "--street", help="(optional, display only) Street & House No. of the POI", nargs="?", default=None
    )
    sendpoi_parser.add_argument(
        "--city", help="(optional, display only) City of the POI", nargs="?", default=None)
    sendpoi_parser.add_argument(
...
async def fingerprint(args) -> None:
    ...
    time_dir = Path.home() / "vehicle_fingerprint" / \
            ...
            try:
                if vehicle.has_electric_drivetrain:
                    await client.get(
                        ...
                        params={"fields": "charging-profile",
...
async def vehicle_finder(args) -> None:
    ...
    print(status.state)
    print({"gps_position": vehicle.status.gps_position,
