...
async def remote_lock(account: MyBMWAccount, args) -> None:
...
async def remote_unlock(account: MyBMWAccount, args) -> None:
    """Trigger the vehicle to lock."""
    await account.get_vehicles()
    vehicle = get_vehicle_or_return(account, args.vin)
...
async def climatize(account: MyBMWAccount, args) -> None:
